...
<html lang="es">
	...
	<body>
		<div class="md-modal md-effect-11" id="niveles"> 
		<form  action="<?=URL::to('catalogos/registrarnivelprograma'); ?>" class="md-content" method="post">
			<h3>Agregar Nivel Programa</h3>
			<div class="tblCatalogos">
				<table class="tblCatPlan">
					...
					<tr>
						<td>Nombre:</td>
						<td><input style="width: 200px; text-transform:uppercase; height: 30px; border-radius: 5px; border-color: #DBDBEA;"  type="text" name="nivelprogramas_descripcion" onkeypress="Mayusculas(this)" id="txtCatDescripcionEtapas" size=1 /></td>
					</tr>
				</table>
			</div>
...
<div class="md-overlay"></div>
		...
		<section><div>
						...
						<div id="adminCatalogos">
							...
							<div id="cmbBuscarCatalogo">
								...
								<select class="con_estilo" style="width:170px;" name="buscarCatalogo" id="seleccion">
									...
									<option value="nivelprograma" selected="selected">Nivel Programa</option>
								</select>		
							</div>
						</div>
					...
					<div class="titulo">Nivel programa</div>
					</div>
	...
	<div id="GridPlanEstudio">
				...
				<div id="btnImprimirBit">
					<input style="width:120px" class="md-trigger" data-modal="niveles" class="estilo_button2" type="button" value="Agregar" name="Agregar" id="">
				</div>
	<br>
			...
			<table cellpadding="0" cellspacing="0" border="0" class="display" id="tblUA">
				...
				<tbody>
						...
						@foreach ($nivelprogramas as $elnivel)
						<tr class="gradeX">
							<form id="{{$elnivel['nivel']}}" method="post" action="<?=URL::to('catalogos/actualizarnivelprograma'); ?>"><td>{{$elnivel['nivel']}}</td><td>
								<input type ="hidden" name="nivelprogramas_id" value="{{$elnivel['nivel']}}"/><div style=" visibility: hidden;">{{$elnivel['descripcion']}}</div>
								<input type="text"id="etapases" name="nivelprogramas_descripcion" id="txtCatDescripcionetapas" value="{{$elnivel['descripcion']}}" readonly="true" ondblclick="this.readOnly='';" onchange="this.form.submit()">
							</td>
						</form>
						
						<td><form method="post" action="<?=URL::to('catalogos/eliminarnivelprograma'); ?>">
							<input type ="hidden" name="nivelprogramas_id" value="{{$elnivel['nivel']}}"/>
