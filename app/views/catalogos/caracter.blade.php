...
<html lang="es">
	...
	<body>
		<div class="md-modal md-effect-11" id="caracter"> 
		<form  action="<?=URL::to('catalogos/registrarcaracter'); ?>" class="md-content" method="post">
			...
			<div class="tblCatalogos">
				<table class="tblCatPlan">
					...
					<tr>
						<td>Nombre:</td>
						<td><input style="width: 200px; text-transform:uppercase; height: 30px; border-radius: 5px; border-color: #DBDBEA;"  type="text" name="caracteres_descripcion" onkeypress="Mayusculas(this)" id="txtCatDescripcionCaracteres" size=1 /></td>
					</tr>
				</table>
			</div>
...
<div class="md-overlay"></div>
	...
	<div id="GridPlanEstudio">
				...
				<div id="btnImprimirBit">
					<input style="width:120px" class="md-trigger" data-modal="caracter" class="estilo_button2" type="button" value="Agregar" name="Agregar" id="">
				</div>
	<br>
			...
			<table cellpadding="0" cellspacing="0" border="0" class="display" id="tblUA">
				...
				<tbody>
						...
						@foreach ($caracteres as $elcaracter)
						<tr class="gradeX">
							<form id="{{$elcaracter['caracter']}}" method="post" action="<?=URL::to('catalogos/actualizarcaracter'); ?>"><td>{{$elcaracter['caracter']}}</td><td>
								<input type ="hidden" name="caracteres_id" value="{{$elcaracter['caracter']}}"/><div style=" visibility: hidden;">{{$elcaracter['descripcion']}}</div>
								<input type="text"id="caracteres" name="caracteres_descripcion" id="txtCatDescripcionCaracter" value="{{$elcaracter['descripcion']}}" readonly="true" ondblclick="this.readOnly='';" onchange="this.form.submit()">
							</td>
						</form>
						
						<td><form method="post" action="<?=URL::to('catalogos/eliminarcaracter'); ?>">
							<input type ="hidden" name="caracteres_id" value="{{$elcaracter['caracter']}}"/>
