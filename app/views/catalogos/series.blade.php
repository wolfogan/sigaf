...
<html lang="es">
	...
	<body>
		...
		<form  action="<?=URL::to('catalogos/registrarseriacion'); ?>" class="md-content" method="post">
		...
		<section><div><div style="display:inline-block; float:left;">Series</div>
						...
						<div id="adminCatalogos">
							...
							<div id="cmbBuscarCatalogo">
								...
								<select class="con_estilo" style="width:170px;" name="buscarCatalogo" id="seleccion">
									...
									<option value="series" selected="selected">Seriación</option>
	...
	<div id="GridPlanEstudio">
				...
				<div id="btnImprimirBit">
					<input style="width:120px" class="md-trigger" data-modal="seriacion" class="estilo_button2" type="button" value="Agregar" name="Agregar" id="">
				</div>
	<br>
			...
			<table cellpadding="0" cellspacing="0" border="0" class="display" id="tblUA">
				...
				<tbody>
						...
						@foreach ($seriaciones as $series)
						<tr class="gradeX">
							<form id="{{$series['reqseriacion']}}" method="post" action="<?=URL::to('catalogos/actualizarseriacion'); ?>"><td>{{$series['reqseriacion']}}</td><td>
								<input type ="hidden" name="seriacion_id" value="{{$series['reqseriacion']}}"/><div style=" visibility: hidden;">{{$series['descripcion']}}</div>
								<input type="text"id="seriacion" name="seriacion_descripcion" id="txtCatDescripcionSeriacion" value="{{$series['descripcion']}}" readonly="true" ondblclick="this.readOnly='';" onchange="this.form.submit()">
							</td>
						</form>
						<script type="text/javascript">
$("#div{{$series['reqseriacion']}}").click(function(){
    $("#{{$series['reqseriacion']}}").prop("disabled", false).focus();
    
});</script>
						<td><form method="post" action="<?=URL::to('catalogos/eliminarseriacion'); ?>">
							<input type ="hidden" name="seriacion_id" value="{{$series['reqseriacion']}}"/>
