		...
		<form  action="<?=URL::to('catalogos/registrarpais'); ?>" class="md-content" method="post">
			...
			<div class="tblCatalogos">
				<table class="tblCatPlan">
					...
					<tr>
						<td>Nombre:</td>
						<td><input style="width: 200px; text-transform:uppercase; height: 30px; border-radius: 5px; border-color: #DBDBEA;" onkeypress="Mayusculas(this)" type="text"  name="paises_descripcion" id="txtCatDescripcionSeriacion" size=1 /></td>
					</tr>
				</table>
			</div>
	...
	<br>
			...
			<table cellpadding="0" cellspacing="0" border="0" class="display" id="tblUA">
				...
				<tbody>
						...
						<tr class="gradeX">
							<form id="{{$elpais['pais']}}" method="post" action="<?=URL::to('catalogos/actualizarpais'); ?>"><td>{{$elpais['pais']}}</td><td>
								<input type ="hidden" name="paises_id" value="{{$elpais['pais']}}"/><div style=" visibility: hidden;">{{$elpais['descripcion']}}</div>
								<input type="text"id="paises" name="paises_descripcion" id="txtCatDescripcionPais" value="{{$elpais['descripcion']}}" readonly="true" ondblclick="this.readOnly='';" onchange="this.form.submit()">
							</td>
						</form>
						<script type="text/javascript">
$("#div{{$elpais['pais']}}").click(function(){
    $("#{{$elpais['pais']}}").prop("disabled", false).focus();
    
});</script>
						<td><form method="post" action="<?=URL::to('catalogos/eliminarpais'); ?>">
							<input type ="hidden" name="paises_id" value="{{$elpais['pais']}}"/>
